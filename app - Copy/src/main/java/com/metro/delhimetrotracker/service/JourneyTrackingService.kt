...
class JourneyTrackingService : LifecycleService() {
    ...
    private fun startJourneyTracking(tripId: Long) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                stationRoute = path.distinctBy { it.stationName }
    ...
    private fun handleManualJump(targetStationId: String, timeOffset: String) {
        ...
        // Create list of all station IDs up to the jump point to fill UI gaps
        val updatedVisitedList = stationRoute.subList(0, targetIndex + 1).map { it.stationId }

        lifecycleScope.launch(Dispatchers.IO) {
            // FIXED: Changed trip.tripId to trip.id
    ...
    private fun handleStationDetection(method: DetectionMethod, confidence: Float, location: Location? = null) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                if (distance[0] > 200f) return@launch // 200m threshold
                ...
                // Haptics for the stop before destination
                if (currentStationIndex == stationRoute.size - 2) triggerVibration()

                lastDetectedStationId = nextStation.stationId
                currentStationIndex++

                // Update Trip DB - FIXED: Changed tripId to id
                val updatedVisited = currentTrip!!.visitedStations.toMutableList().apply { add(nextStation.stationId) }
    ...
    private fun stopJourneyTracking() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            currentTrip?.let { trip ->
                ...
                // FIXED: Changed trip.tripId to trip.id
                database.tripDao().completeTrip(trip.id, TripStatus.COMPLETED, endTime, duration)
                val lastVisitedStationId = trip.visitedStations.lastOrNull()
                val actualEndStation = if (lastVisitedStationId != null) {
                    stationRoute.find { it.stationId == lastVisitedStationId }?.stationName
            ...
            withContext(Dispatchers.Main) {
                // Send broadcast to notify TrackingActivity
    ...
    private fun createNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Metro Journey Active")
            .setContentText("${currentTrip?.sourceStationName ?: "Tracking"} to ${currentTrip?.destinationStationName ?: "Destination"}")
